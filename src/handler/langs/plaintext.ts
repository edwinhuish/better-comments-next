import type { PickDecorationOptionsParams } from './common';
...
export class PlainTextHandler extends Handler {
  protected async updateDecorations(editor: vscode.TextEditor): Promise<void> {
    ...
    for (const visibleRange of editor.visibleRanges) {
      ...
      await pickDecorationOptions({ editor, text, offset, tagRanges });
    }
    ...
    await pickDecorationOptions({ editor, text, offset: 0, tagRanges });
...
async function pickDecorationOptions({ editor, text, offset, tagRanges, processed = [] }: PickDecorationOptionsParams) {
